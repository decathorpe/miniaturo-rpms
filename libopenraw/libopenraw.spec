...
# remove vendored Rust dependencies
rm -r lib/mp4/vendor/
# set up build environment for cargo
%cargo_prep
rm lib/mp4/Cargo.lock
...
%files gnome-devel
%{_libdir}/%{name}gnome.so
%{_libdir}/pkgconfig/%{name}-gnome-0.3.pc

%dir %{_includedir}/%{name}-0.3/%{name}-gnome
%{_includedir}/%{name}-0.3/%{name}-gnome/gdkpixbuf.h

%files pixbuf-loader
%{_libdir}/gdk-pixbuf-2.0/*/loaders/*.so

%changelog
* Sat Apr 29 2023 Julian Sikorski <belegdol@fedoraproject.org> - 0.3.5-1
- Update to 0.3.5
